...
def run(key):
    ...
    if data_file:
        ...
        if mean_result:
            ...
            df_cal = df.groupby(['specimen', 'strain']).agg({"s_index":["count","mean","median","min","max","std","var"]})
            st.write(df_cal) 

            df['group'] = df['specimen'].str.cat(df['strain'], sep='')
            ...
            agg_df = df.groupby(['specimen', 'strain'], as_index=False).agg({'s_index':['mean', 'std']})
            agg_df.columns = ['specimen', 'strain', 'mean', 'sd']
            agg_df['SE'] = agg_df['sd'] / np.sqrt(df.groupby(['specimen', 'strain']).size().values)

            fig = px.bar(agg_df, x=('specimen'), y='mean', error_y='SE', color='strain', barmode='group', range_y=(-0.20, 0.20), width=600, height=400, color_discrete_sequence=px.colors.sequential.Blues[4:]).update_layout(
            ...
            fig2 = px.violin(df, x='specimen', y='s_index', box=True, points='all', color='strain', violinmode='group', range_y=(-0.75, 0.75), width=600, height=400, color_discrete_sequence=px.colors.sequential.Blues[6:]).update_layout(
                xaxis_title='Cancer type', yaxis_title='Index(-)', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
            ...
            df_pivot = pd.pivot_table(df, values='s_index', index='strain', columns='specimen')
            ...
            features = ['specimen', 'strain', 's_index']   

            X = df[features]
            y = df['s_index']

            X_enc = pd.get_dummies(X, columns=['specimen', 'strain'])
            ...
            pca_df['strain'] = X['strain']

            fig = px.scatter_3d(pca_df, x='PC1', y='PC2', z='PC3', color='specimen',
                                symbol='strain', hover_name='specimen',
                                hover_data={'strain': True, 'PC1': False, 'PC2': False, 'PC3': False})
            st.plotly_chart(fig)

            subset_df = df.loc[df['strain'].isin(['1_10(1)', '1_10(2)', '1_10(3)','1_10(4)'])]

            fig = px.scatter_3d(subset_df, x='strain', y='specimen', z='s_index', color='strain', symbol='strain')
            fig.update_layout(scene=dict(xaxis_title='strain', yaxis_title='specimen', zaxis_title='s_index'))
